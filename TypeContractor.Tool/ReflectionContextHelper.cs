...
{
	internal static MetadataLoadContext GetMetadataContext(string packPath, int dotnetVersion, string assemblyPath, ILog logger)
	{
		return new(GetResolver(packPath, dotnetVersion, assemblyPath, logger));
	}

	internal static PathAssemblyResolver GetResolver(string packPath, int dotnetVersion, string assemblyPath, ILog logger)
	{
		...
		var netcoreDirectory = GetNetCorePack(packPath, "Microsoft.NETCore.App.Ref", dotnetVersion)
			?? throw new FileNotFoundException($"Unable to find Microsoft.NETCore.App.Ref v{dotnetVersion}.0.x references. Searched in {packPath}.");
		...
		var aspnetDirectory = GetNetCorePack(packPath, "Microsoft.AspNetCore.App.Ref", dotnetVersion)
			?? throw new FileNotFoundException($"Unable to find Microsoft.AspNetCore.App.Ref v{dotnetVersion}.0.x references. Searched in {packPath}.");
	...
	internal static string? GetNetCorePack(string packPath, string packName, int dotnetVersion)
	{
		...
		var availablePacks = Directory.EnumerateDirectories(packPrefix, $"{dotnetVersion}.0.*");
		var packDirectory = availablePacks.OrderByDescending(x => x).FirstOrDefault();
		if (string.IsNullOrWhiteSpace(packDirectory))
			return null;

		packDirectory = Path.Combine(packDirectory, "ref", $"net{dotnetVersion}.0");
